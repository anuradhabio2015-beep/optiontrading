...
# PAGE CONFIGURATION
# --------------------------------------------------------------
st.set_page_config(layout="wide", page_title="Smart Option Selling Dashboard — Gemini Pro Dynamic")
st.title("🤖 Smart Option Selling Dashboard — Gemini Pro (Dynamic)")
...
with st.sidebar:
    ...
    selected_universe = st.multiselect(
        ...
        help="Choose symbols for AI analysis"
    )
...
if not selected_universe:
    st.warning("⚠️ Please select at least one symbol from the Universe.")
    st.stop()

# --------------------------------------------------------------
# RUN AI SELECTION (on button click)
# --------------------------------------------------------------
if run_ai or "ai_selection" not in st.session_state:
    with st.spinner("🤖 Running Gemini AI model to pick symbols..."):
        st.session_state["ai_selection"] = ai_select_stocks_gemini(selected_universe)
        st.session_state["ai_summary"] = ai_market_summary_gemini(st.session_state["ai_selection"])

# --------------------------------------------------------------
# DISPLAY AI RESULTS + METRICS DYNAMICALLY
# --------------------------------------------------------------
selection = st.session_state["ai_selection"]
st.subheader("🧠 AI Selection Output")
st.json(selection)
...
for row in selection:
    ...
    st.subheader("🧮 Strategy Ideas (Reactive to Filters)")
